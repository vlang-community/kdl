...
fn test_basic_node() {
	doc := kdl.parse('node 1 "two" three key=#true')!
...
fn test_accessors() {
	doc := kdl.parse('server host="localhost" port=8080 { tls #true; route "/a"; route "/b" }')!
	s := doc.get('server')?
...
fn test_numbers() {
	doc := kdl.parse('n 42 -7 +3 1_000 0xFF -0o17 0b1010 1.5 -2.5e3 1e-2 0.1_5 #inf #-inf #nan')!
...
fn test_integer_bounds() {
	doc := kdl.parse('n 9223372036854775807 -9223372036854775808 9223372036854775808 -9223372036854775809 0xABCDEF0123456789ABCDEF')!
	a := doc.nodes[0].arguments
	...
	assert (a[4].data as kdl.BigInt).str() == '207698809136909011942886895'
...
fn test_subnormal_float() {
	doc := kdl.parse('n 1.23e-308 5e-324')!
...
fn test_invalid_numbers() {
	...
	for src in ['n 1__0', 'n 1_', 'n 1.0_', 'n 1e1_0', 'n 0x1_', 'n 0xa_b', 'n -', 'n +', 'n .',
		'n .foo', 'n -.foo', 'n +-1', 'n --1'] {
		kdl.parse(src) or { assert false, 'should accept ${src}: ${err.msg()}' }
	}
}

fn test_keywords() {
	doc := kdl.parse('n #true #false #null')!
	a := doc.nodes[0].arguments
	...
	doc2 := kdl.parse('n truex nullable infinity')!
	assert doc2.nodes[0].arguments.len == 3
}

fn test_strings_and_escapes() {
	doc := kdl.parse(r'n "a\"b\\c\n\r\t\b\f\s" "\u{1F600}" "x\    y" #"raw \n "# ##"has "# inside"##')!
	a := doc.nodes[0].arguments
...
fn test_type_annotations() {
	doc := kdl.parse('(person)node (u8)1 key=(date)"2024" ("")x ( spaced )y (t/*c*/)z')!
	n := doc.nodes[0]
	assert n.ty? == 'person'
...
fn test_properties_last_wins_and_order() {
	...
	if _ := kdl.parse('n 1=2') {
		assert false
	}
}

fn test_children_and_terminators() {
	doc := kdl.parse('a { b; c { d }; e }\nf {}; g\n"h"{i}')!
	...
	if _ := kdl.parse('a { b; c { d } e }') {
		assert false
	}
	// zero space before `{` and `;` is fine
	doc5 := kdl.parse('n a{b}\nn a;b')!
...
fn test_comments() {
	...
	doc2 := kdl.parse('node//comment')!
	assert doc2.nodes[0].name == 'node'
	if _ := kdl.parse('node /* unterminated') {
		assert false
	}
}

fn test_slashdash() {
	doc := kdl.parse('/- gone 1\nn /- 1 2 /-k=3 k=4 /-{ x } { y } /- { z }\n/-\nm')!
	assert doc.nodes.len == 1
	n := doc.nodes[0]
...
fn test_invalid_utf8() {
	...
	if _ := kdl.parse(surrogate) {
		assert false
	}
	overlong := [u8(`n`), ` `, 0xC0, 0xAF].bytestr()
	if _ := kdl.parse(overlong) {
		assert false
	}
}
...
fn test_version_marker() {
	...
	assert doc.nodes.len == 1
	if _ := kdl.parse('/- kdl-version 1\nnode') {
		assert false
	}
	// without a trailing newline it is just a slashdashed node
	doc2 := kdl.parse('/- kdl-version 2')!
	assert doc2.nodes.len == 0
}

fn test_empty_documents() {
	for src in ['', '\n', '  ', '// c', '/* c */', '/- n', '\\\n'] {
		doc := kdl.parse(src) or { panic('${src}: ${err.msg()}') }
...
fn parse_error_of(src string) kdl.ParseError {
	kdl.parse(src) or { return err as kdl.ParseError }
	panic('`${src}` parsed')
}

fn test_error_column_ignores_the_bom() {
	err_plain := parse_error_of('node "a')
	err_bom := parse_error_of('\ufeffnode "a')
	assert err_plain.line == 1 && err_bom.line == 1
	assert err_bom.col == err_plain.col
	assert err_bom.offset == err_plain.offset + 3 // offsets stay byte offsets of the text
	err_line2 := parse_error_of('\ufeffok\nnode "a')
	assert err_line2.line == 2 && err_line2.col == err_plain.col
}
