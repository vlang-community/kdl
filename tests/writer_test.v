...
const f64_nan = math.nan()

// format_float returns how the writer renders a float without type annotation.
...
fn test_writer_basic() {
	doc := kdl.parse('(t)node 1 -2 2.5 1e10 "a b" bare #true #null key="v" { child; "x y" (u8)0xff }')!
...
fn test_writer_floats() {
	...
	doc := kdl.parse('n 1e21 1.5e-7 0.1 100.0')!
	assert kdl.parse(doc.str())!.nodes[0].arguments == doc.nodes[0].arguments
}

fn test_writer_escapes_unicode_newlines() {
	doc := kdl.parse('n "\\u{85}\\u{2028}\\u{2029}\\u{b}\\u{c}" "\\u{85}"=1')!
	text := doc.str()
	assert text == 'n "\\u{85}\\u{2028}\\u{2029}\\u{b}\\f" "\\u{85}"=1\n'
	assert kdl.parse(text)!.equals(doc)
}
...
fn test_zero_value_is_null_and_round_trips() {
	...
	n.arguments << kdl.Value{}
	n.properties['k'] = kdl.Value{}
	assert n.str() == 'n #null k=#null\n'
	assert kdl.parse(n.str())!.nodes[0].equals(n)
}
